...
class RequestParserTestCase(unittest.TestCase):
    ...
    pattern = {
        ...
        '22': {
            ...
            'table': {
                ...
                '8' : 'swat_vip_badkill',   # SWAT victory in VIP Escort - Suspects have killed the VIP
                '9' : 'sus_vip_badkill',    # Suspects victory in VIP Escort - SWAT have killed the VIP
        ...
        '27': {
            ...
            'item': {
                ...
                'items': {
                    ...
                    '35': {
                        'type': node.NumericPatternNode,
                        'name': 'enemy_incaps_invalid',
                        'required': False,
                        'default': '0',
                    },
                    '36': {
                        'type': node.NumericPatternNode,
                        'name': 'enemy_kills_invalid',
                        'required': False,
                        'default': '0',
                    },
                    '37': {
    ...
    SAMPLE_DOT = os.path.join(SAMPLE_DIR, 'dot.txt')
    SAMPLE_ARRAY = os.path.join(SAMPLE_DIR, 'array.txt')

    def setUp(self):
        self.pattern_node = shortcuts.parse_pattern(self.pattern)

    def test_dot_samples(self):
        with open(self.SAMPLE_DOT) as f:
            for value in f:
                value = value.strip()
                if not value:
                    continue
                qs = shortcuts.julia_v2(value)
                self.pattern_node.parse(qs)

    def test_array_samples(self):
        with open(self.SAMPLE_ARRAY) as f:
            for value in f:
                value = value.strip()
                if not value:
                    continue
                qs = shortcuts.julia_v1(value)
                self.pattern_node.parse(qs)